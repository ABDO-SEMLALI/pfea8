 <?php
                ...
                if($username&&$email&&$password&&$repeatpassword){
                    $usernamelength=strlen($username);
                        ...
                        if(filter_var($email,FILTER_VALIDATE_EMAIL)){
                            $recemail=$bdd -> prepare("SELECT * FROM encadrant WHERE email_enc=?");
                            ...
                            if($emailexiste==0){
                                ...
                                $handler = mysqli_query($base, "SELECT id_enc FROM encadrant ORDER BY id_enc DESC LIMIT 1");
   ...
   $insertuser= $bdd ->prepare("INSERT INTO encadrant(id_enc,nom_complet,email_enc,password,nom_fil) VALUES(?,?,?,?,?)");
   $insertuser -> execute(array($id,$username,$email,$password,$fil));
   echo "Le compte a bien ete cree";
   header('location:add_enc.php');
  }
...
-->
 ...
 <div class="user-container">
        ...
        <div class="info-container">
            <div class="set-edit">
                <span> information de l'encadrant</span>
                

            </div>

            <div class="info">
              <form action="add_enc.php" method="POST" >
                <label></label>
